...
var_summary_2 <- function(column, na.rm = TRUE) {
...
# apply the var_summary function to numeric columns in the df
#var_summary_2(priv_gs_focus$perc_garden)

# priv_gs_focus %>%
#   names() %>% 
#   purrr::map(var_summary_2, .data[[.x]])

variable_name <- priv_gs_focus %>%
  select_if(is.numeric) %>% 
  names()

summary_priv_gs_focus <- priv_gs_focus %>%
  select_if(is.numeric) %>% 
  purrr::map_dfr(var_summary_2) %>% 
  add_column(variable_name, .before = 1)



# summary_priv_gs_focus$variable <- num_col_names

summary_priv_gs_focus
