...
for (i in seq_along((probabilities))){
  ...
  processed_alt_lsoa[col_name] <- processed_alt_lsoa["cases"]

  # replace na with c(0, 1, 2) according to probabilities defined above
  processed_alt_lsoa[[col_name]][is.na(processed_alt_lsoa[[col_name]])] <-
    sample(
      c(0,1,2),
      size = sum(is.na(processed_alt_lsoa[[col_name]])),
...
processed_alt_lsoa <- processed_alt_lsoa %>%
  # tidy data
  relocate(week_commencing, .after = week) %>%
  rename(source_cases = cases) %>%
  pivot_longer(cols = source_cases:ncol(processed_alt_lsoa), values_to = "cases", names_to = "processing") %>% 
...
#View(processed_alt_lsoa_tidy)


#cases_per_wk_processing_options_tidy <- get_cases_by_week(processed_alt_lsoa_tidy, na.rm = FALSE, processing)
View(processed_alt_lsoa_tidy)
