...
```{r}
processed_headline_2 <- processed_headline_1 %>% 
...
```{r}
processed_lsoa_3 <- processed_lsoa_1 %>% 
  select(-week)

processed_headline_3 <- processed_headline_2 %>% 
...
```{r, echo = FALSE}
head(processed_lsoa_4)
head(processed_headline_4)
```
...
```{r}
cpw_lsoa <- get_cases_per_week(df = processed_lsoa_4, case_col = cases_per_week)
cpw_headline <- get_cases_per_week(df = processed_headline_4, case_col = cases_per_day)
...
```{r}
processed_lsoa_alt <- processed_lsoa_4
...
for (i in seq_along((probabilities))){
  ...
  processed_lsoa_alt[col_name] <- processed_lsoa_alt["cases_per_week"]

  # replace na with c(0, 1, 2) according to probabilities defined above
  processed_lsoa_alt[[col_name]][is.na(processed_lsoa_alt[[col_name]])] <-
    sample(
      c(0,1,2),
      size = sum(is.na(processed_lsoa_alt[[col_name]])),
...
processed_lsoa_alt <- processed_lsoa_alt %>%
  ...
  pivot_longer(cols = source_cases:ncol(processed_lsoa_alt), values_to = "cases", names_to = "processing") %>% 
  
  # calculate case numbers per week (for each processing approach)
  get_cases_per_week(na.rm = TRUE, processing) #%>% 
...
#View(processed_lsoa_alt_tidy)


#cases_per_wk_processing_options_tidy <- get_cases_by_week(processed_lsoa_alt_tidy, na.rm = FALSE, processing)
View(processed_lsoa_alt_tidy)

# Facet plot the alternative approaches to processing NA (coded as -99 in source data)values
ggplot(data = processed_alt_lsoa_tidy, mapping = aes(
