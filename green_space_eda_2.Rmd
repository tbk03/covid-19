...
total_cases_income <- total_cases_focus %>% 
  ...
  mutate(above_mean = Mean > eng_mean_salary,
         above_median = Median > eng_median_salary) %>% #create variable to allow grouping in plots below
...
# create plot using Mean as the measure of centre for salary data
p4a <-  ggplot(data = subset(total_cases_income, subset = ave_garden_size < 1000), # remove an outlier
             mapping = aes(x = ave_garden_size)
             )

p4a + green_space_covid_plot() +
...
green_space_covid_plot_median <- function(){
...
p4a + green_space_covid_plot_median() +
  
  facet_wrap(~ above_median, labeller = as_labeller(facet_labels)) +
...
```{r, fig.height=6}
# 
# x <-  quantile(total_cases_income$Median, c(0.25, 0.5, 0.75))
# total_cases_income$quartile <- cut(total_cases_income$Median, x, include.lowest = TRUE)
total_cases_income <- total_cases_income %>% 
  mutate(median_salary_quartile = ntile(Median, 4))

green_space_covid_plotter <- function(df, x_var, sal_ave, facet_var){
  ...
  facet_wrap(vars(.data[[facet_var]]), labeller = as_labeller(facet_labels))

}

p <- green_space_covid_plotter(df = subset(total_cases_income, 
...
p1 <- green_space_covid_plotter(df = subset(total_cases_income,
