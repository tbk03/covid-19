...
For England 
median gross salary 26,055	
mean gross salary 32,237
...
x_label <- expression("Average garden size in"~m^2)
y_label <- "Total number of confirmed Covid-19 cases"
subtitle <- str_c("The average salary across England in 2020 is ", eng_mean_salary)
title <- "Holding title"
legend_label <- "..."
...
p4 <-  ggplot(data = subset(total_cases_income, subset = ave_garden_size < 1000),
             mapping = aes(x = ave_garden_size,
                           y = cases_per_ht,
                           fill = Mean)
             )

p4 + geom_jitter(size = 2, shape = 21, colour = "grey30") +
...
```{r}
p5 <-  ggplot(data = total_cases_income,
             mapping = aes(x = ave_dist_to_park, 
                           y = cases_per_ht,
                           fill = Mean)
             )

p5 + geom_jitter(size = 2, shape = 21, colour = "grey30") +
