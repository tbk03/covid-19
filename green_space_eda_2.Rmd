...
total_cases_income <- total_cases_focus %>% 
  ...
  mutate(diff_from_mean = Mean - eng_mean_salary)
...
limit <- max(total_cases_income$diff_from_mean, na.rm = T) * c(-1, 1)

p4 <-  ggplot(data = total_cases_income,
             mapping = aes(x = ave_garden_size, 
                           y = cases_per_ht,
                           fill = Mean)
             )

p4 + geom_jitter(size = 2, shape = 21, colour = "grey50") +
